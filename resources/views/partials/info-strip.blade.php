@php
  ...
  $showPhone = ($attrs['showPhone'] ?? true) !== false;
  $showAddress = ($attrs['showAddress'] ?? true) !== false;
  $showHours = ($attrs['showHours'] ?? true) !== false;
  $stripClass = function_exists('App\\wr_band_section_class')
    ? \App\wr_band_section_class($attrs, 'info-strip')
    : 'info-strip';
@endphp
<div class="{{ esc_attr($stripClass) }}">
  <div class="wrap">
    @if ($showAddress)
    <div class="info-item">
    ...
    <div class="info-item">
      ...
      <a href="{{ esc_url(Identity::phoneHref()) }}">{{ Identity::phone() }}</a>
    </div>
    @endif
  </div>
</div>
